...
async function run(): Promise<void> {
  const sql = `
    ...
    -- =====================================================================
    -- HOMEPAGE SECTIONS (May-2026 cinematic CMS — Phase 1 + Phase 3)
    -- =====================================================================
    -- Trainer-editable cinematic content for hero / philosophy / final-CTA
    -- sections. Image as base64 data URL (same rationale as profile_picture_url
    -- and hero_images.image_data_url — Vercel read-only fs). NULL columns
    -- everywhere so the frontend's static fallbacks always have a safe path.
    -- Schema shape matches an existing earlier-session table on the prod
    -- DB exactly (cta_*_href not _url, overlay_opacity / blur_intensity
    -- as INTEGER 0-100 percent / px not double precision, object_position
    -- defaults "center center", orphan media_asset_id column preserved).
    -- Aligning here avoids destructive ALTER COLUMN on live data.
    ...
    -- Idempotent seed: only insert default rows on first boot. ON CONFLICT
    -- DO NOTHING means re-running ensureSchema NEVER overwrites admin edits.
    -- Copy is intentionally minimal — frontend renders inline t(key, fallback)
    -- strings until the admin populates the CMS. Sort order matches the
    -- master-prompt cinematic flow (1=hero, 50=mid-page, 99=final).
    INSERT INTO homepage_sections (key, eyebrow, title, subtitle, sort_order) VALUES
      ('hero', 'PREMIUM PERSONAL TRAINING', NULL, NULL, 1),
      ('philosophy', 'PHILOSOPHY', NULL, NULL, 50),
      ('final_cta', 'BEGIN', NULL, NULL, 99)
