...
- **P5g Performance Sprint — Batch 1: route-level code splitting (May 2026)** — `client/src/App.tsx` converted from 30 eager `import` statements to `React.lazy()` + `<Suspense fallback={<Loader/>}>`. Eager kept for the critical path: `HomePage` (LCP for guests), `AuthPage` (auth funnel), `NotFound` (fallback that must always be available). All 28 other pages now ship as on-demand chunks. **Initial JS: 3.6MB → 1.9MB (-47%)**. Per-page chunks: ClientDashboard 635K (recharts-heavy), LineChart vendor split 385K, AdminClientDetail 101K, most others <50K. Zero logic changes; routing/auth/translations unchanged. `vite.config.ts` not touched.
