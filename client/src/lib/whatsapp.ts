...
// =============================
// Coach-Curated Protocol request builder (Phase A, May 2026)
// =============================
//
// Builds a short, premium WhatsApp message a guest or signed-in client can
// send to Coach Youssef to request access to one of the 3 public tiers
// (Essentials / Performance / Concierge). Self-contained mini-phrasebook
// — does NOT extend the existing Phrasebook type so a missing translation
// for a new language never becomes a build break.

export type ProtocolTierLabel = "Essentials" | "Performance" | "Concierge";

const PROTOCOL_REQUEST_PHRASES: Record<string, (tier: string, name: string) => string> = {
  en: (tier, name) =>
    `${name ? `Hello Coach Youssef, this is ${name}.` : "Hello Coach Youssef,"}\n\nI'd like to request the ${tier} Protocol. When you have a moment, please let me know the next steps.\n\nThank you.`,
  ar: (tier, name) =>
    `${name ? `مرحبًا كوتش يوسف، أنا ${name}.` : "مرحبًا كوتش يوسف،"}\n\nأرغب في طلب بروتوكول ${tier}. عندما يكون لديك وقت، أرجو إعلامي بالخطوات التالية.\n\nشكرًا لك.`,
  fa: (tier, name) =>
    `${name ? `سلام مربی یوسف، من ${name} هستم.` : "سلام مربی یوسف،"}\n\nمایلم پروتکل ${tier} را درخواست کنم. وقتی فرصت داشتید، لطفاً مراحل بعدی را اطلاع دهید.\n\nمتشکرم.`,
  ur: (tier, name) =>
    `${name ? `السلام علیکم کوچ یوسف، میں ${name} ہوں۔` : "السلام علیکم کوچ یوسف،"}\n\nمیں ${tier} پروٹوکول کی درخواست کرنا چاہتا/چاہتی ہوں۔ جب آپ کو وقت ہو، براہ کرم اگلے اقدامات بتائیں۔\n\nشکریہ۔`,
  de: (tier, name) =>
    `${name ? `Hallo Coach Youssef, hier ist ${name}.` : "Hallo Coach Youssef,"}\n\nIch möchte das ${tier}-Protokoll anfragen. Wenn du Zeit hast, gib mir bitte die nächsten Schritte.\n\nVielen Dank.`,
  fr: (tier, name) =>
    `${name ? `Bonjour Coach Youssef, c'est ${name}.` : "Bonjour Coach Youssef,"}\n\nJe souhaite demander le Protocole ${tier}. Quand tu auras un moment, merci de m'indiquer les prochaines étapes.\n\nMerci.`,
  ru: (tier, name) =>
    `${name ? `Здравствуйте, тренер Юссеф, это ${name}.` : "Здравствуйте, тренер Юссеф,"}\n\nЯ хотел(а) бы запросить протокол ${tier}. Когда будет минутка, сообщите, пожалуйста, дальнейшие шаги.\n\nСпасибо.`,
};

export interface BuildProtocolRequestOptions {
  /** Display label for the tier, e.g. "Essentials" / "Performance" / "Concierge". */
  tierLabel: string;
  lang?: string | null;
  clientName?: string | null;
}

export function buildProtocolRequestWhatsApp(opts: BuildProtocolRequestOptions): string {
  const code = (opts.lang || "en").slice(0, 2).toLowerCase();
  const fn = PROTOCOL_REQUEST_PHRASES[code] ?? PROTOCOL_REQUEST_PHRASES.en;
  return fn(opts.tierLabel, (opts.clientName || "").trim());
}
