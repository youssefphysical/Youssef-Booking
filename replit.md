...
- **OI1 Client Health Score** — derived per-client status badge with priority logic and 0-100 score. Statuses: `healthy | watch | at_risk | inactive | new | frozen | ended` (each maps to a calm tone — emerald/amber/rose/slate/sky/cyan/zinc). New `server/services/clientHealth.ts → computeClientHealth(s: HealthSignals)` is a pure function (frozen-package or frozen-status returns frozen; ended status short-circuits; otherwise scores from base 100 deducting for missed weeks / no-shows / missing check-in / missing body metric, with softer decay for new clients). New `storage.getHealthSignalsForUsers(userIds)` runs **5 batched grouped queries** (max(date) for completed bookings + weekly_checkins + body_metrics; count of completed/no_show in last 30d split by status; active package frozen flag) and returns `Map<userId, signals>` — O(1) DB queries regardless of N. `sanitizeAndEnrich` + `sanitizeAndEnrichMany` accept `{ withHealth?: boolean }`; only `/api/users` (admin list) and `/api/users/:id` (when `me.role === 'admin'`) set true so the badge **never leaks** to clients themselves via `BookingWithUser` / `PackageWithUser` etc. New `<HealthBadge>` (`client/src/components/HealthBadge.tsx`) — subtle dot+optional label pill with tooltip showing top signal (e.g. "Last session 18 days ago · 2 no-shows in 30d"), `xs|sm` sizes. Mounted in `AdminClients` ClientRow (xs, dot-only) + MobileCard (xs with label) + `AdminClientDetail` ClientHeader (sm). Architect PASS first try (with two minor polish fixes applied: also detect frozen packages even if user `clientStatus` not synced; gentler new-client decay).
