...
- **Onboarding + Approval (May 2026):** Register form has an optional package picker (`packageTemplateId`). Server snapshots the chosen template into a `packages` row with `adminApproved=false` + `paymentStatus='unpaid'`, sets `clientStatus='pending'`, and writes an admin notification (`kind='system'`). The trainer reviews from `/admin/pending` (Approve flips client to `active` + marks the snapshotted package paid+approved with a `package_session_history` entry; Reject sets client `cancelled` + deactivates pending packages). Booking is gated by `evaluateBookingEligibility` (returns `client_pending` while awaiting approval). `general_fitness` is now an allowed `primaryGoal`.
